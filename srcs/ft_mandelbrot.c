...
/*   Updated: 2023/06/26 16:33:44 by cscelfo          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void    *ft_mandelbrot(void *thread_d)
{
    ...
    while (data->coord.px < thread_data->x_end)
    {
        ...
        while (data->coord.py < data->small_side)
        {
            ...
            data->color = (int)((smoothed_iter * (data->color_man * 10) / data->coord.max_iter)) % 256;
...
void    ft_mandelbrot_decorator(t_data *data)
{
    ...
    while (++i < NUM_THREADS)
    {
        ...
        pthread_create(&threads_creator[i], NULL, ft_mandelbrot, (void *)(&thread_data[i]));
    }
    i = -1;
    while (++i < NUM_THREADS)
